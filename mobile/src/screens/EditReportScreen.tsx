...
type Route = RouteProp<RootStackParamList, "EditReport">;
...
export default function EditReportScreen() {
  ...
  const mutation = useMutation({
    mutationFn: (data: Parameters<typeof reportsApi.updateReport>[1]) =>
    ...
    onSuccess: () => {
      ...
      Alert.alert("Сохранено!", "Отчёт успешно обновлён.", [
        { text: "OK", onPress: () => nav.goBack() },
      ]);
    },
    onError: (e: any) => {
      Alert.alert("Ошибка", e?.response?.data?.detail || "Не удалось сохранить изменения");
    },
  });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {step === "confirm" && (
        <StepCard title="Подтверждение изменений">
